...
#define IRQ_MODE_TO_IRQC(mode)                                                 \
  ((mode) == GPIO_IRQ_MODE_DISABLE ? PORT_eDisabled : (0x08 | (mode)))
...
static const IRQn_Type port_irqs[] = {PORTA_IRQn, PORTB_IRQn, PORTC_IRQn,
...
static gpio_port_callbacks
    irqCallbacks[PORT_NUM]; // here we will store the callback
...
bool gpioIRQ(pin_t pin, uint8_t irqMode, pinIrqFun_t irqFun) {
  ...
  port_ptrs[port]->PCR[portPin] =
      (port_ptrs[port]->PCR[portPin] & ~PORT_PCR_IRQC_MASK) |
      PORT_PCR_IRQC(IRQ_MODE_TO_IRQC(irqMode));

  irqCallbacks[port].callbacks[irqCallbacks[port].used].callback_fn = irqFun;
  irqCallbacks[port].callbacks[irqCallbacks[port].used].pin =
      PIN2NUM(pin);          // Obs! port pin is stored
...
void PORTA_IRQHandler(void) { execute_callbacks(PA); }
void PORTB_IRQHandler(void) { execute_callbacks(PB); }
void PORTC_IRQHandler(void) { execute_callbacks(PC); }
void PORTD_IRQHandler(void) { execute_callbacks(PD); }
void PORTE_IRQHandler(void) { execute_callbacks(PE); }
